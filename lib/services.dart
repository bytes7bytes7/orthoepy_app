...
Future<File> appendFile(String key, int value) async {
  ...
  final file = await localFile();
  return file.writeAsString(sortWords(map));
}
...
Future<File> switchFile(String key) async {
  Map<String, int> map = await readFile();
  map[key] = (map[key]-1).abs();
  final file = await localFile();
  return file.writeAsString(sortWords(map));
}


String sortWords(Map<String, int> map) {
...
List<String> shuffleAccent(String word) {
  ...
  for (int i = 0; i < word.length; i++) {
    if (kVowels.contains(word[i])) {
      lst.add(
          word.substring(0, i) + word[i].toUpperCase() + word.substring(i + 1));
    } else if (word[i] == '(') {
      break;
    }
  }
